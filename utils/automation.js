...
const automation = {
	run_autotrader_script(){
		...
		return new Promise(resolve=>{
			...
			(async()=>{
				...
				let command = shell.exec('node /workspace/automation_scripts/autotrader/autotrader.js ', {async:true});
	...
	async run_adesa_script(){
		...
		return new Promise((resolve)=>{
			...
			let command = shell.exec('node /workspace/automation_scripts/adesa/adesa-saved-search.js > ./logs/adesa-logs.txt ', {async:true});
			command.on('exit',function(code){
				if(code === 0){
					// console.log('finished with success');
	...
	async run_airtable_script(){
		...
		return new Promise((resolve)=>{
			...
			let command = shell.exec('node /workspace/automation_scripts/adesa/airtable.js > ./logs/airtable-logs.txt ', {async:true});
			command.on('exit',function(code){
				if(code === 0){
					// console.log('finished with success');
