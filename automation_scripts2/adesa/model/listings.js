...
Listings.prototype.addOrUpdateListing = async function (listing, options) {
    ...
    const oldListing = await Listings.findOne({listing_id: listing.listing_id}, options);

    if (!oldListing) {
        return Listings.create(listing, options);
    } else {
        return Listings.update(listing, options);
    }
}
